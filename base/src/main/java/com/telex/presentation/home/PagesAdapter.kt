...
class PagesAdapter(
    ...
    val onItemClickListener: (Page, EditorMode) -> Unit,
    ...
    override fun onBindViewHolder(h: RecyclerView.ViewHolder, position: Int) {
        when (getItemViewType(position)) {
            TYPE_ITEM -> {
                ...
                holder.bind(page)
                holder.itemView.setOnClickListener { onItemClickListener.invoke(page, EditorMode.Edit) }
                ...
                holder.itemView.setOnLongClickListener {
                    PageOptionsFragment.newInstance(
                        ...
                        editOption.onClick = { onItemClickListener.invoke(page, EditorMode.Edit) }
                    }.also {
    ...
    class ItemViewHolder(var context: Context, itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(page: Page) {
            ...
            itemView.authorTextView.text = page.authorName
            if (!page.title.isNullOrBlank()) {
                itemView.titleTextView.text = page.title
            } else {
                itemView.titleTextView.text = context.getString(R.string.untitled)
            }
            itemView.viewsTextView.text = page.views.toString()
            if (imageUrl == null || imageUrl.isEmpty()) {
                itemView.pageImageView.visibility = View.GONE
            } else {
                itemView.pageImageView.visibility = View.VISIBLE
                val size = context.resources.getDimension(R.dimen.height_item_page).toInt()
                itemView.pageImageView.loadImage(
                        context = context,
